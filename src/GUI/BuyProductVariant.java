...
public class BuyProductVariant extends JDialog{
    ...
    public BuyProductVariant(Window window,Product p,int userid)
    {
                    ...
                    if(cart!=null)
                    {
                        if(cart.getQuantity()+buyquan<pv.getQuantity())
                        {
                            if(cartbus.updateCartQuantity(userid, pv, cart.getQuantity()+buyquan))
                                JOptionPane.showMessageDialog(BuyProductVariant.this, "Sản phẩm đã có sẵn trong giỏ hàng, đã cập nhật số lượng","Thông báo",JOptionPane.INFORMATION_MESSAGE);
                        }
                    }
                    else 
                    {
                        if(pv.getQuantity()>buyquan)
                        {
                            if(cartbus.addProductToCart(pv, userid, buyquan)){
                                JOptionPane.showMessageDialog(BuyProductVariant.this, "Thêm vào giỏ hàng thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                        }
                        }
                    }
